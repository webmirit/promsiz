					...
					<div class="section_item_inner">
						<div class="img">
							<?if(is_array($arSection['PICTURE']) && $arSection['PICTURE']['SRC']):?>
								...
								<a href="<?=$arSection['SECTION_PAGE_URL']?>" class="thumb"><img src="<?=$img['src']?>.webp" alt="<?=($arSection['PICTURE']['ALT'] ? $arSection['PICTURE']['ALT'] : $arSection['NAME'])?>" title="<?=($arSection['PICTURE']['TITLE'] ? $arSection['PICTURE']['TITLE'] : $arSection['NAME'])?>" /></a>
							<?elseif($arSection['~PICTURE']):?>
								<?$img = CFile::ResizeImageGet($arSection['~PICTURE'], array( "width" => 120, "height" => 120 ), BX_RESIZE_IMAGE_EXACT, true );?>
								<a href="<?=$arSection['SECTION_PAGE_URL']?>" class="thumb"><img src="<?=$img['src']?>.webp" alt="<?=$arSection['NAME']?>" title="<?=$arSection['NAME']?>" /></a>
							<?else:?>
								<a href="<?=$arSection['SECTION_PAGE_URL']?>" class="thumb"><img src="<?=SITE_TEMPLATE_PATH?>/images/no_photo_medium.png.webp" alt="<?=$arSection['NAME']?>" title="<?=$arSection['NAME']?>" /></a>
							<?endif;?>
						</div>
